...
import { competencesDeveloppeur } from '@/data/content';
import { motion } from 'framer-motion';
import DevStack from '@/components/sections/Stack';

export default function Skills() {
  return (
    <section className="py-20 px-4 max-w-6xl mx-auto" id="competences">
      <motion.div
      ...
      >
        <h2 className="text-3xl md:text-5xl font-bold mb-4 bg-clip-text text-transparent bg-gradient-to-r from-purple-500 to-pink-500">
          {competencesDeveloppeur.title}
        </h2>
        <p className="text-gray-400 text-lg">{competencesDeveloppeur.subtitle}</p>
      ...
      >
        <div>
          <h2 className="text-3xl font-bold mb-6 text-white"><span className="text-primary">Mes</span> Compétences Développeur</h2>
          ...
          <div className="flex flex-wrap gap-2">
            {competencesDeveloppeur.subjects.map((skill) => (
        ...
        <div className="h-[300px] md:h-[400px] w-full bg-white/5 rounded-2xl border border-white/10 backdrop-blur-sm p-4">
          <ResponsiveContainer width="100%" height="100%">
            <RadarChart cx="50%" cy="50%" outerRadius="70%" data={competencesDeveloppeur.subjects}>
