// components/ui/TunnelBackground.tsx
"use client";
...
export default function TunnelBackground() {
  ...
  // 1. Vitesse d'avancée : Plus on scroll, plus la valeur Z augmente
  // On avance de 0px à 1000px vers la caméra
  const zPosition = useTransform(scrollYProgress, [0, 1], [0, 1000]);
  
  // 2. Disparition : Reste opaque jusqu'à 85% de la page, puis fade out complet à 95%
  // Cela permet de laisser la section Contact (à la fin) sur fond noir propre
  ...
  // Création de 20 "portes" pour le tunnel
  const layers = Array.from({ length: 20 }, (_, i) => ({
    id: i,
    z: -i * 150, // Chaque porte est reculée de 150px
    scale: 1 + i * 0.1, // Légère perspective forcée
  }));

  return (
    <motion.div 
      className="fixed inset-0 z-[-1] bg-[#050505] overflow-hidden"
      style={{ opacity }} // Applique la disparition progressive
    >
      {/* Point de fuite (Centre du tunnel) avec un dégradé pour la profondeur */}
      ...
      <div className="absolute inset-0 flex items-center justify-center [perspective:1000px]">
        ...
        >
          {layers.map((layer, index) => {
            // Alternance des couleurs pour l'effet Cyberpunk (Violet, Bleu, Rose)
            ...
            return (
              <div
                ...
                style={{
                  ...
                  borderRadius: '20px', // Coins légèrement arrondis comme sur l'image
                }}
              >
                {/* Lignes de connexion (Optionnel: ajoute des détails "Tech") */}
